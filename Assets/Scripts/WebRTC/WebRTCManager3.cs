...
public class WebRTCManager3 : MonoBehaviour
{
    
        // Entrei -> crio a oferta e envio pra API

        // Um background job rodando para procurar ofertas, caso encontre, gera uma resposta e envia pra API

        // Um background job rodando para procurar repostas, caso encontre, define a description remotamente.

        // Precisa de uma flag para identificar quais já foram pra não realizar o mesmo processo.
    ...
    private IEnumerator CreateOffer(){
        ...
        if(isHost)
            File.WriteAllText(@"C:\webrtc\offer.txt", offerDescription.sdp);
    }
    ...
    private IEnumerator CreateAnswer(){
        ...
        if(!isHost)
            File.WriteAllText(@"C:\webrtc\answer.txt", answerDescription.sdp);
    }

    private IEnumerator AddAnswer(){
        string answerFileContent = "";
        if(isHost)
            answerFileContent = File.ReadAllText(@"C:\webrtc\answer.txt");

        var answer = new RTCSessionDescription { sdp = answerFileContent, type = RTCSdpType.Answer };
