...
function _ConnClose(peerInstance, connInstance) {
 var peer = UnityPeerJS.peers[peerInstance];
 var conn = peer.conns[connInstance];
 conn.close();
}

function _Connect(peerInstance, id, roomName) {
...
function _GetLocalPeerID(peerInstance) {
 var peer = UnityPeerJS.peers[peerInstance];
...
function _OpenPeer() {
 var peer = {
  peer: new Peer(null, {
   host: "proton-server.squareweb.app",
   debug: 2
  }),
  initialized: false,
  conns: [],
...
var asmLibraryArg = {
 "ConnClose": _ConnClose,
 "Connect": _Connect,
 "GetLocalPeerID": _GetLocalPeerID,
 "Init": _Init,
...
if (!Object.getOwnPropertyDescriptor(Module, "UnityPeerJS")) Module["UnityPeerJS"] = function() {
 abort("'UnityPeerJS' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");
};
